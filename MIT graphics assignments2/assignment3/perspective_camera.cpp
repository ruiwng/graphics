...
PerspectiveCamera::PerspectiveCamera(Vec3f center, Vec3f direction, Vec3f up, float angle) :center(center), direction(direction), up(up), angle(angle){
	this->direction.Normalize();
}

Ray PerspectiveCamera::generateRay(Vec2f point) {
	Vec3f screenPoint = origin + horizontalDir * point.x() + verticalDir * point.y();
...
void PerspectiveCamera::initRayTracing() {
	Vec3f z = direction;
	z.Negate();
	Vec3f::Cross3(horizontalDir, up, z);
	horizontalDir.Normalize();
	Vec3f::Cross3(verticalDir, z, horizontalDir);

	Vec3f screenCenter = center - 0.5f / tan(angle * 0.5f) * z;
	origin = screenCenter - horizontalDir * 0.5f - verticalDir * 0.5f;
}
