...
OrthographicCamera::OrthographicCamera(Vec3f center, Vec3f direction, Vec3f up, float size): center(center), direction(direction), up(up), size(size){
	this->direction.Normalize();
}

Ray OrthographicCamera::generateRay(Vec2f point) {
	Vec3f o = origin + horizontalDir * point.x() * size + this->verticalDir * point.y() * size;
...
void OrthographicCamera::initRayTracing() {
	Vec3f z = direction;
	z.Negate();
	Vec3f::Cross3(horizontalDir, up, z);
	horizontalDir.Normalize();
	Vec3f::Cross3(verticalDir, z, horizontalDir);
	origin = center - horizontalDir * size * 0.5f - verticalDir * size * 0.5f;
}
