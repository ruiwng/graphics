...
public:
	...
	void drawJoints( );
	void drawJointsRecursively(const Joint* root);

	// 1.2. Implement this method a recursive helper to draw a box between each pair of joints
	void drawSkeleton( );
	void drawSkeletonRecursively(const Joint* root);
	...
	void computeBindWorldToJointTransformsRecursively(Joint* root, const Matrix4f& m);
	// 2.3.2. Implement this method to compute a per-joint transform from
	// joint space to world space in the CURRENT POSE.
	void updateCurrentJointToWorldTransforms();
	void updateCurrentJointToWorldTransformsRecursively(Joint* root, const Matrix4f& m);
